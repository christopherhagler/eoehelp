...
def _allergen_array() -> postgresql.ARRAY[Any]:
    return postgresql.ARRAY(pg_enum(AllergenGroup, "allergen_group"))
...
class CatalogIngredient(Base):
    ...
    # The same identity a label ingredient gets (food/products/vocabulary.py), so
...
class FoodProduct(UUIDPrimaryKey, Base):
    ...
    source: Mapped[FoodDataSource] = mapped_column(
        pg_enum(FoodDataSource, "food_data_source"), nullable=False
    )
...
class FoodLogItem(UUIDPrimaryKey, Timestamps, Base):
    ...
    meal: Mapped[Meal] = mapped_column(pg_enum(Meal, "meal"), nullable=False)
...
class FoodLogItemIngredient(UUIDPrimaryKey, Base):
    ...
    provenance: Mapped[IngredientProvenance] = mapped_column(
        pg_enum(IngredientProvenance, "ingredient_provenance"), nullable=False
    )
