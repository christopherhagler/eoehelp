...
| 8 | implementer | Stage 3 built, minus the registry cache | CI now runs the checks **inside the images it builds**, which is ADR 0006's premise and was previously false: the `api` job `pip install`ed on the runner and the `images` job built images nothing executed. Four jobs, three of them on `ubuntu-24.04-arm` so nothing is cross-built or emulated; `openapi-drift` disappears as a job and becomes a step using the same script a developer runs. A composite action installs podman, buildah, skopeo and a checksum-verified `just` tarball rather than a third-party action. `just ci-api`, `ci-web` and `ci-images` are the exact sequences, runnable locally — all three verified here before pushing. Three script seams were needed first: `openapi.sh` exports the schema from the image rather than through `compose exec`, so it needs no running stack (which also makes `just contract-check` work with the stack down); `api-checks.sh` gained a `migrations` subcommand and now gets its image from `build-images.sh`, closing the "two answers to where an image comes from" the devops reviewer recorded; and the AppArmor `unshare` restriction on Ubuntu 24.04 is relaxed in the composite action, because podman and skopeo ship no profile while buildah does — the reason `ci.yml` already documented skopeo failing there. **Deliberately not in this commit: `--cache-from`/`--cache-to`.** Registry-backed caching needs GHCR authentication and `packages: write`, which belongs with the publish workflow in stage 4; landing the "CI runs the images" half alone keeps the revert clean, as *Staged delivery* asks. The cost is that each job builds cold until stage 4. |
