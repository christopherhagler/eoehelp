...
2. **A stored version must stay reproducible.** Files are never edited after
   ...
   > **Corrected 2026-09-21 — this paragraph was wrong, and the defect it
   > describes happened.** The registry and the files change in the same
   > commit, so editing a published document and updating its digest passes
   > `verify_integrity()` silently; and `GET /legal/documents/{id}` always
   > returns a version's *current* bytes, so a consent row could point at
   > wording nothing could produce. Twenty-seven development consent rows did.
   > The fix — a `legal_documents` ledger written by migration, a composite
   > foreign key from `consents`, digest-addressed document reads, and a
   > startup reconciliation — is designed in
   > [2026-09-21-consent-text-reproducibility.md](2026-09-21-consent-text-reproducibility.md).
   > Read that plan, not this paragraph, for how publishing works.
...
| 6 | code-reviewer | FIX REQUIRED | Two blocking. **A slug resolved to the oldest version of a document:** `by_id` returned the first registry match and `DOCUMENTS` is ordered oldest-first, so publishing `tos-2026-11` would have left `/terms` — the footer link, the sign-in link and the onboarding dialog — serving superseded wording while `onboarding_service.complete()` recorded the *new* digest against the patient's consent, which is the exact failure this feature exists to prevent. The escape hatch made it worse: the old uniqueness test would have failed on a second version and steered the fix towards renaming the public URL. Now a version id resolves exactly, a slug resolves through `current_for`, and the uniqueness test asserts what the router actually needs (slugs never collide with version ids; one consent type never carries two slugs), with a new test that registers a second version and checks both halves. **`content_sha256` missing from the consent audit metadata** and three named tests never written — both fixed, see rows below. Advisories fixed: emphasis crossing a link silently inverted the bolding after the link and now raises; table header cells and captions bypassed every parser refusal and now go through the same plain-text rule; `"avoid "` restored to the forbidden-wording guard; the nine arbitration sub-clauses asserted individually; a test ties the registry digests to migration 0006's literals; `document_id` carries `Path(max_length=64, pattern=...)` like every other path parameter; the web client encodes the id; `LegalDocument` reacts to `paramMap` instead of reading the snapshot once, so following the superseded notice reloads the text. Advisories deferred with reason: the `REVIEWED_CACHE` branch test, the remaining web spec gaps, ISO date formatting, the unused `MatDialogRef`, the double `current_for` in the synthetic writer, and `welcome.ts` field ordering. |
| 7 | security-reviewer | FINDINGS | One high, verified live against the running stack. **Rate limits were keyed on the resolved URL, not the route:** slowapi falls back to `request.url.path` when no scope is given, so `/legal/documents/terms` returned 429 while `/legal/documents/privacy` and five random ids returned 200, and every distinct id minted a new Redis key against a `noeviction` store — a path to filling it and disabling *every* limiter, including the magic-link one. The same defect made `PRODUCT_LOOKUP` inert, so one account could make unbounded outbound calls to Open Food Facts and USDA. Fixed with `ratelimit.route_limit`, which always passes an explicit scope, applied to all nine rate-limited routes, plus an AST test that fails if a bare `limiter.limit` returns and a regression test that exhausts one document and asserts the next request for a different one is also refused. **Log injection and resolved paths in logs:** uvicorn's access log and slowapi's logger print the decoded path and client address, which let the reviewer forge a convincing log line through a crafted URL — and made the privacy policy's "the route pattern rather than the address you visited" untrue. `--no-access-log` in the image, those loggers silenced in `configure_logging`, structlog middleware left as the only request log. **`//evil.test` and `/\evil.test` passed the parser's internal-path check** and would have been bound to `routerLink`; tightened to `INTERNAL_PATH` and mirrored in `legal-content.ts` rather than trusting the server. Also fixed: the unencoded route parameter, and `Vary: Origin` now set on every response, without which a shared cache would blank the legal pages once a document is marked reviewed. Routed to the build-and-deploy plan rather than fixed here: the Host-header redirect (needs the deployed hostnames), and `ENVIRONMENT=staging` on a production host silently disabling both the draft gate and the production secret checks. Noted and not acted on: `review_status` is a one-token edit with no reviewer name or date behind it. |
| 8 | database-reviewer | FINDINGS | Two blocking, both verified against the live database. **`consents` was append-only in a docstring only:** migration 0001 grants `app_runtime` UPDATE and DELETE on every table but `audit_log`, and as that role the reviewer successfully ran `UPDATE consents SET document_sha256 = …, granted = false` and `DELETE FROM consents` (rolled back). This commit is what made it matter, because the row now carries the digest that *is* the evidence. Migration 0006 now revokes both, with the matching grant in `downgrade`, and `tests/db/test_consent_grants.py` asserts it; the reviewer had already proved a revoke does not break account deletion, since the cascade runs as the constraint owner and ignores grants and RLS. **Half the consent rows in the development database carry digests of text that no longer exists** — 27 rows pointing at the pre-legal-review wording under the same three version ids, because `verify_integrity` compares the file to the registry and both change together. That is a design problem, so it went to the architect rather than into improvised code; see the row below. Advisories fixed: migration 0006 now names the uncovered versions instead of failing with a bare NOT NULL violation, passes `existing_type`, and says in its docstring that a downgrade destroys legal evidence rather than rolling anything back; `content_sha256` added to the consent audit metadata, which is the only record that survives account deletion; `document_sha256` exposed on `ConsentRecord`, which the privacy policy and section 18.7 of the terms both promise a patient can obtain; constraint-rejection tests added for uppercase, short, long, non-hex, empty and padded digests, and a test that onboarding stores the registry's digest rather than merely a well-formed one. Confirmed and unchanged: the check constraint binds exactly what it claims (uppercase, padded and newline-terminated values all rejected), `varchar(64)` is the right type, the index still serves the only query the new code issues, and `consents` must never be pruned on a schedule. Deferred: `research_consent_scopes` has no RLS and no `patient_id`, so the catalog test passes it by omission — with the architect. |
| 9 | implementer | Fixed, all checks green | Everything in rows 6 to 8 that was not routed elsewhere. Checks: 579 API tests, 63 web tests, ruff, mypy, contract and web types regenerated (`ConsentRecord.document_sha256` and the `document_id` pattern), web build. The revoke was also applied to the development database by hand, because it already sat at 0006. Outstanding and with the architect: digest immutability, and `research_consent_scopes`. |
| 10 | architect (revision after the database reviewer) | Superseded in part; see the follow-up plan | The database reviewer found, and verified against the development database, that this plan's central guarantee did not exist: `consents.document_sha256` pins bytes nothing enforces, because the registry digest and the file change together, `/legal/documents/{id}` only ever serves a version's current bytes, and 27 of 54 development consent rows already pointed at pre-legal-review wording that was never committed and cannot be reproduced. That is a data-model defect rather than a wording one, so it is **its own plan**: `docs/plans/2026-09-21-consent-text-reproducibility.md`, which adds a `legal_documents` ledger populated by migration, a composite foreign key from `consents (consent_type, document_version, document_sha256)`, revision-aware registry entries with digest-addressed reads, a startup reconciliation between the registry and the ledger, and the publishing rules that decide when a correction is a new revision and when it is a new version. It also closes the reviewer's second finding — `research_consent_scopes` had no RLS policy and no `patient_id`, so the isolation test passed by omission. This plan is annotated where it was wrong (the *Where the text lives* section) rather than rewritten, because it is the record of what was built. Its deferred items — re-consent, the account screen, the withdrawal path — stay deferred; the follow-up says what each will inherit. |
